...
(*      numbers as {!Cstruct.t}. *1) *)
(*  module type S = sig *)

(*    (1** {1 Base} *)

(*        Type [t] with the basic bit-twiddling related operations. *1) *)
...
(*    (1** {1 External representation} *1) *)

(*    val bit_bound : t -> bits *)
(*    (1** [bit_bound t] computes the upper bound of {{!bits}[bits]} quickly. *1) *)

(*    val pp_print : Format.formatter -> t -> unit *)
(*    (1** [pp_print ppf t] pretty-prints [t] on [ppf]. *1) *)

(*    val bits : t -> bits *)
(*    (1** [bits t] is the minimal number of bits needed to describe [t]. *)

(*        [(2^(bits t)) / 2 <= t < 2^(bits t)]. *1) *)

(*    val of_cstruct_be : ?bits:bits -> Cstruct.t -> t *)
(*    (1** [of_cstruct_be ~bits cs] interprets the bit pattern of [cs] as a *)
(*        {{!t}[t]} in big-endian. *)
...
(*    (1** [to_cstruct_be ~size t] is the big-endian representation of [t]. *)
...
(*        If it is smaller, the high bits in [t] are dropped. *1) *)

(*    val into_cstruct_be : t -> Cstruct.t -> unit *)
(*    (1** [into_cstruct_be t cs] writes the big-endian representation of [t] into *)
(*        [cs]. It behaves like {{!to_cstruct_be}[to_cstruct_be]}, with [~size] *)
(*        spanning the entire [cs]. *1) *)
(*  end *)
...
(*  (1** {1 Misc elementary number theory} *1) *)

(*  val pseudoprime : Z.t -> bool *)
(*  (1** Miller-Rabin with sane rounds parameter. *1) *)

...
let sign ~key:({ p; q; g; x; _ } as priv) ?(mask = `Yes) ?k ?outbits m =
  ...
  let rec tryme () =
    ...
    let key, k, inv_k, m =
      match mask with
      | `No  -> (priv, k, Z.(invert k q), m)
      | `Yes -> let blind = Rng.Z.gen_r Z.one q in
                ({ priv with x = Z.(x * blind) }, k, Z.(invert (blind * k) q), Z.(m * blind))
    in
    match sign_ key k inv_k m with
    | None -> tryme ()
    | Some (r, s) ->
       let size =
         match outbits with
         | None   -> None
         | Some n -> Some (cdiv n 8)
       in
       Numeric.Z.(to_cstruct_be ?size r, to_cstruct_be ?size s)
  in
  tryme ()

let verify ~key:({ p ; q ; g ; y } : pub) m (r, s) =
  let r = Numeric.Z.of_cstruct_be r
  and s = Numeric.Z.of_cstruct_be s
  and m = Numeric.Z.of_cstruct_be m
  in
  if r > Z.zero && s > Z.zero && r < q && s < q then
    let w = Z.(invert s q) in
    let u1 = Z.(m * w mod q) in

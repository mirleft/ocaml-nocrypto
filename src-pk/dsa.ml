...
let params ?g size =
  ...
  let q = RngZ.prime ?g ~msb:1 n in
  let p =
    let q_q  = Z.(q * ~$2) in
    until pseudoprime @@ fun () ->
      let x = RngZ.gen_bits ?g ~msb:1 l in
      Z.(x - (x mod q_q) + one) in
  let gg =
    let e = Z.(pred p / q) in
    until ((<>) Z.one) @@ fun () ->
      let h = RngZ.gen_r ?g Z.(~$2) Z.(pred p) in
      Z.(powm h e p) in
  (p, q, gg)

let generate ?g size =
  let (p, q, gg) = params ?g size in
  let x = RngZ.gen_r ?g Z.one q in
...
module K_gen (H : Hash.S) = struct
  ...
  let z_gen ~key:{ q; x; _ } z =
    let repr = Repr.(to_cstruct_be ~size:(bit_size q // 8)) in
    let g    = Rng.create ~strict:true drgb in
    Rng.reseed ~g Cs.(repr x <+> repr Z.(z mod q));
    RngZ.gen_r ~g Z.one q

  let generate ~key cs =
    z_gen ~key Repr.(of_cstruct_be ~bits:(bit_size key.q) cs)
end
...
let sign ?mask ?k ~(key : priv) digest =
  let bits   = Repr.bit_size key.q in
  let size   = bits // 8 in
  let (r, s) = sign_z ?mask ?k ~key (Repr.of_cstruct_be ~bits digest) in
  Repr.(to_cstruct_be ~size r, to_cstruct_be ~size s)

let verify ~(key : pub) (r, s) digest =
  let z      = Repr.(of_cstruct_be ~bits:(bit_size key.q) digest)
  and (r, s) = Repr.(of_cstruct_be r, of_cstruct_be s) in
  verify_z ~key (r, s) z

let massage ~key:({ q; _ }: pub) digest =
  let bits = Repr.bit_size q in
  if bits >= Cstruct.len digest * 8 then digest else
    let cs = Repr.(to_cstruct_be Z.(of_cstruct_be digest mod q)) in

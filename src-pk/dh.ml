...
let modulus_size { p; _ } = Repr.bit_size p
...
let key_of_secret_z ({ p; gg; _ } as group) x =
  ...
  | ggx -> ({ x }, Repr.to_cstruct_be ggx)

let key_of_secret group ~s =
  key_of_secret_z group (Repr.of_cstruct_be s)
...
let rec gen_key ?g ?bits ({ p; q; _ } as group) =
  let pb = Repr.bit_size p in
  let s = Option.(
    imin (get_or exp_size pb bits)
         (q >>| Repr.bit_size |> get ~def:pb))
    |> RngZ.gen_bits ?g ~msb:1 in
